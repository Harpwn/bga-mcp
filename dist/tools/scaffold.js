...
export const scaffoldTools = [
    {
        ...
        description: "Generate a PHP State class file for modules/php/States/. This is the modern BGA approach (replaces states.inc.php array entries).",
        inputSchema: {
            type: "object",
            properties: {
                className: {
    ...
    },
    {
        ...
        description: "Generate an #[PossibleAction] method stub to add inside a BGA State class.",
        inputSchema: {
            type: "object",
            properties: {
                actionName: {
    ...
    },
    {
        ...
        description: "Generate PHP (backend) and JS (frontend) stubs for a BGA notification.",
        inputSchema: {
            type: "object",
            properties: {
                notifName: {
    ...
    },
    {
        ...
        description: "Generate the skeleton file structure and boilerplate content for a new BGA game project using the modern State classes approach.",
        inputSchema: {
            type: "object",
            properties: {
                gameName: {
...
export async function handleScaffoldTool(name, args) {
...
// ---------------------------------------------------------------------------
function generateStateClass(args) {
    const className = args.className;
    const gameName = args.gameName;
    const id = args.id;
    const rawType = args.type ?? "ACTIVE_PLAYER";
    const stateType = `StateType::${rawType}`;
    const description = args.description ?? `\${actplayer} must do something`;
    const descriptionMyTurn = args.descriptionMyTurn ?? `\${you} must do something`;
    const transitions = args.transitions ?? {};
    const actions = args.actions ?? [];
    const updateGameProgression = args.updateGameProgression ?? false;
...
function generateStateAction(args) {
...
function generateNotification(args) {
    const notifName = args.notifName;
    const notifArgs = args.args ?? [];
...
function scaffoldGame(args) {
    const gameName = args.gameName.toLowerCase();
    const gameNamePascal = ucfirst(gameName);
    const minPlayers = args.playerCount?.min ?? 2;
    const maxPlayers = args.playerCount?.max ?? 4;
    const files = [
        {
            path: `Game.php`,
