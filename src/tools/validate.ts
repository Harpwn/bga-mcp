...
export const validateTools: Tool[] = [
  {
    ...
    description:
      "Lint a local BGA game project for common bugs and inconsistencies: broken transition targets, missing zombie() methods, JS↔PHP action name mismatches, duplicate state IDs, unreachable states, mismatched filenames vs class names, and more.",
    inputSchema: {
      type: "object",
      properties: {
...
export async function handleValidateTool(
  ...
  if (name === "bga_validate_project") return validateProject(args);
...
interface StateInfo {
  id: number;
  name: string;
  type: string;  // "activeplayer" | "multipleactiveplayer" | "game" | "manager" | "ACTIVE_PLAYER" etc.
  transitions: Record<string, number>;  // label → target id
  possibleactions: string[];            // action names (modern: without "act" prefix, legacy: with)
  hasZombie: boolean;
  source: "states.inc.php" | "StateClass";
...
function parseStatesInc(content: string, relPath: string): StateInfo[] {
...
function parseStateClass(content: string, relPath: string): StateInfo | null {
  ...
  if (!idMatch || !typeMatch || !nameMatch) return null;
...
function isPlayerStateType(type: string): boolean {
 ...
 */
function normaliseAction(name: string): string {
...
function runChecks(
  ...
  const idCounts = new Map<number, StateInfo[]>();
  for (const s of states) {
    if (!idCounts.has(s.id)) idCounts.set(s.id, []);
  ...
  const referencedIds = new Set<number>([RESERVED_INITIAL, RESERVED_END]);
  for (const s of states) {
  ...
  for (const s of states) {
    if (s.source !== "StateClass" || !s.className) continue;
  ...
  const jsFile = files.find(
    (f) => f.endsWith(".js") && !f.includes(".min.") && f !== "Gruntfile.js"
  );
  ...
  if (jsFile) {
    ...
    const bgaActionNames = new Set<string>();
    ...
    const phpActions = new Set<string>();
    for (const s of states) {
    ...
    const gamePhp = ["Game.php", ...files.filter((f) => f.endsWith(".game.php"))].find(
  ...
  if (jsFile) {
    ...
    const phpNotifs = new Set<string>();
    ...
    for (const notif of phpNotifs) {
      if (notif === "message" || notif === "tableWindow") continue; // BGA built-ins
  ...
  if (fs.existsSync(statesDir)) {
    for (const s of states) {
      if (s.source !== "StateClass") continue;
...
function validateProject(args: Record<string, unknown>) {
  ...
  const rel = (p: string) => path.relative(projectPath, p).replace(/\\/g, "/");
  ...
  function formatGroup(items: Diagnostic[], heading: string) {
  ...
  const summary = [
    ...
    statusLine,
    "",
    `Scanned **${states.length}** state definition(s) across ${
      fs.existsSync(statesIncPath) ? "`states.inc.php`" : ""
    }${fs.existsSync(statesIncPath) && fs.existsSync(statesDir) ? " + " : ""}${
      fs.existsSync(statesDir) ? `\`modules/php/States/\`` : ""
    }.`,
    "",
