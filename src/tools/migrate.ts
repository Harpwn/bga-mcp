...
function stateNameToClass(name: string): string {
...
function mapStateType(oldType: string): string {
...
function parseStatesInc(content: string): ParsedState[] {
...
export const migrateTools: Tool[] = [
  {
    ...
    description:
      "Scan a local BGA project and report its migration status: which files still use deprecated patterns (states.inc.php, self::checkAction, notifqueue) vs the modern State classes approach.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Read an existing states.inc.php from a local project and generate a modern PHP State class stub for each state defined in it. Skips the reserved gameSetup (id=1) and gameEnd (id=99) states.",
    inputSchema: {
      type: "object",
      properties: {
        projectPath: projectPathProp,
  ...
  },
  {
    ...
    description:
      "Return a step-by-step guide for migrating an existing BGA game from the legacy states.inc.php + self::checkAction pattern to the modern State classes architecture.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
...
export async function handleMigrateTool(
  name: string,
  args: Record<string, unknown>
) {
  if (name === "bga_migration_status") return migrationStatus(args);
  if (name === "bga_convert_states_inc")  return convertStatesInc(args);
  if (name === "bga_migration_guide")     return migrationGuide();
  return { content: [{ type: "text", text: `Unknown migrate tool: ${name}` }], isError: true };
}

// ---------------------------------------------------------------------------

function migrationStatus(args: Record<string, unknown>) {
  ...
  // ---- Summary -------------------------------------------------------------
  const lines: string[] = [`# Migration Status: \`${gameName}\`\n`];
...
function convertStatesInc(args: Record<string, unknown>) {
  const { resolved: projectPath, gameName } = resolveProjectPath(args);
  if (!projectPath) {
    const games = WORKSPACE_PATH ? discoverGames(WORKSPACE_PATH) : [];
    return noProjectPathError(games.length > 1);
  ...
  const rawPascal = (args.gameNamePascal as string | undefined) ?? ucfirst(gameName ?? "Game");
